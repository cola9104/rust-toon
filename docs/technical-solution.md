...
- 图片任务记录实际发送的提示词、像素尺寸及参考图数量（不保存参考图 Base64）。HTTP 与 Base64 结果均须通过文件解码、宽高比和最低分辨率检查，并成功保存至 MinIO，才能标记任务成功。允许 3% 的比例对齐误差和 5% 的尺寸误差；超宽、低清或损坏结果报失败，不自动替换已有资产图片，也不自动付费重绘。
