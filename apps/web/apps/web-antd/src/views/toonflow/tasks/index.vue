...
<template>
  <Page auto-content-height class="toon-page">
    <Card :bordered="false" class="toonflow-page-card h-full toon-surface">
      ...
      <div class="task-stats"><Card size="small"><span>全部任务</span><b>{{ taskStats.total }}</b></Card><Card size="small" class="running"><span>处理中</span><b>{{ taskStats.running }}</b></Card><Card size="small" class="success"><span>已完成</span><b>{{ taskStats.success }}</b></Card><Card size="small" class="failed"><span>失败</span><b>{{ taskStats.failed }}</b></Card></div>
...
<style scoped>.task-stats{display:grid;margin-bottom:16px;gap:12px;grid-template-columns:repeat(4,minmax(0,1fr))}.task-stats .ant-card{border-left:3px solid #d9d9d9}.task-stats .ant-card.running{border-left-color:#1677ff}.task-stats .ant-card.success{border-left-color:#52c41a}.task-stats .ant-card.failed{border-left-color:#ff4d4f}.task-stats span{display:block;color:#8c8c8c;font-size:12px}.task-stats b{display:block;margin-top:4px;font-size:24px}.task-detail { display: grid; gap: 5px; }.task-detail label { margin-top: 12px; color: #8c8c8c; font-size: 12px; }.task-detail p { margin: 0; }.task-detail pre { padding: 12px; border-radius: 10px; white-space: pre-wrap; background: #f5f5f5; }.task-detail pre.error { color: #a8071a; background: #fff1f0; }</style>
