...
async function generateSelectedVideos(tracks: any[]) {
  ...
  try {
    ...
    return { trackId: track.id, uploadData: videoUploadData(track, mode), prompt: track.prompt || storyboard?.videoDesc || '', duration: track.duration || storyboard?.duration || 4 };
    }), model: first.model || project.value.videoModel, mode, resolution: first.resolution || '1080p', audio: first.audio !== false });
...
async function generateVideo(track:any) {
  ...
  let id: number;
  try {
    id = await generateTrackVideo({ projectId:projectId.value,scriptId:selectedScriptId.value,trackId:track.id,prompt:track.prompt||storyboard?.videoDesc||'',model:options.model||project.value.videoModel,mode,resolution:options.resolution||'1080p',duration:track.duration || storyboard?.duration || 4,audio:options.audio !== false,uploadData:videoUploadData(track, mode) });
  } catch (error) {
