...
const sceneKeyOptions = computed(() => {
  const keys = new Set([
    ...props.sceneKeys.map((key) => key.trim().toLowerCase()).filter(Boolean),
    ...catalog.value.scenes.map((scene) => scene.sceneKey),
  ]);
  if (keys.size === 0) keys.add('sc1');
  return [...keys]
...
const currentMaster = computed(() =>
  catalog.value.scenes.find((scene) => scene.sceneKey === selectedSceneKey.value),
);
...
const referenceAssetOptions = computed(() =>
  props.assets
    ...
    .map((asset) => ({ label: `${asset.name}（${asset.type}）`, value: asset.id })),
);
...
async function loadCatalog() {
  ...
  loading.value = true;
  try {
    catalog.value = await getSceneConsistencyCatalog(props.projectId, props.scriptId);
...
async function submitMaster() {
  if (!props.scriptId) return;
  if (!/^sc[1-9]\d*$/.test(selectedSceneKey.value)) {
    return message.warning('场次键必须使用 scN 格式');
  }
...
function openStateEditor(state?: ToonflowApi.SceneState) {
  ...
  Object.assign(stateForm, {
    ...
    referenceAssetIds: state?.references.map((reference) => reference.assetId) ?? [],
...
async function submitState() {
  ...
  if (!/^[a-z][a-z0-9_-]{0,63}$/.test(stateKey)) {
    return message.warning('状态键必须以小写字母开头，只能含字母、数字、_ 或 -');
  }
  if (
    stateKey !== 'base' &&
    ...
    <Spin :spinning="loading">
      <div class="scene-consistency-layout">
        <Card size="small" title="场次">
          ...
          />
          <p class="scene-help">
            同一时间、同一地点仍属于同一个场次；门、桌等被破坏时新增“状态”，不要新建场次。
          </p>
        </Card>

        <Card size="small" title="空间母版">
          <template #extra>
          ...
          <div class="master-grid">
            <Image
              v-if="currentMaster?.referenceUrl"
              :src="assetFileUrl(currentMaster.referenceUrl)"
              :width="220"
            />
            <Empty v-else :image="Empty.PRESENTED_IMAGE_SIMPLE" description="尚无母版图" />
            <Form layout="vertical">
              <Form.Item label="母版名称">
                <Input v-model:value="masterForm.name" />
              </Form.Item>
              ...
              <Form.Item label="固定空间说明">
                <Input.TextArea
                  v-model:value="masterForm.spatialPrompt"
                  :rows="4"
                  placeholder="例如：门在北墙中央，长桌靠左墙，窗户位于右后方；这些位置跨镜头不变"
                />
              </Form.Item>
              <Button type="primary" :loading="savingMaster" @click="submitMaster">
                保存并锁定母版版本
              </Button>
            </Form>
          </div>
        ...
        <Card size="small" title="场景状态">
          <template #extra>
            <Button :disabled="!currentMaster" type="primary" @click="openStateEditor()">
              新建状态
            </Button>
          </template>
          <Empty
          ...
          <div v-else class="state-list">
            ...
            >
              <div class="state-card__header">
                ...
                <Button size="small" @click="openStateEditor(state)">编辑</Button>
              </div>
              <p>{{ state.changeSummary || (state.stateKey === 'base' ? '以母版中的初始完好状态为准' : '未填写变化说明') }}</p>
              <p v-if="state.statePrompt" class="scene-help">{{ state.statePrompt }}</p>
            </Card>
          </div>
        </Card>
      </div>
      ...
      <div class="state-form-grid">
        <Form.Item label="状态键">
          <Input
            ...
            :disabled="stateForm.id !== undefined && stateForm.stateKey === 'base'"
        ...
        <Form.Item label="基于状态">
          <Select
            ...
            allow-clear
            :disabled="stateForm.stateKey === 'base'"
