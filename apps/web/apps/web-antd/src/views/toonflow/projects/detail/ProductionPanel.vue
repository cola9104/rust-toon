...
const productionSceneKeys = computed(() =>
  [...new Set((props.context.storyboards ?? []).map((item: ToonflowApi.Storyboard) => item.sceneKey).filter(Boolean))] as string[],
);

async function refreshSceneCatalog() {
  if (!props.context.selectedScriptId) {
    sceneCatalog.value = { scenes: [] };
    return;
  }
  sceneCatalog.value = await getSceneConsistencyCatalog(
...
function openStoryboard(item?: ToonflowApi.Storyboard, keepInsertPosition = false) {
  ...
  Object.assign(storyboardForm, {
    ...
    sceneKey: item
      ? (item.sceneKey ?? '')
      : (props.context.storyboards.at(-1)?.sceneKey ?? 'sc1'),
