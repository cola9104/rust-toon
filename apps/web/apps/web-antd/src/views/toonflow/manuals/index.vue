...
const columns=[{title:'名称',dataIndex:'name',width:220},{title:'标识',dataIndex:'path',width:230},{title:'内容项',key:'items',width:100},{title:'参考图',key:'images',width:100},{title:'更新时间',key:'updateTime',width:180},{title:'操作',key:'action',width:150}];
...
<template><Page auto-content-height><Card :bordered="false" class="toonflow-page-card h-full"><template #title>创作手册</template><template #extra><Space><Button @click="load">刷新</Button><Button type="primary" @click="edit()">新建{{ activeKind === 'visual' ? '视觉' : '导演' }}手册</Button></Space></template><Tabs v-model:active-key="activeKind"><Tabs.TabPane key="visual" tab="视觉手册" /><Tabs.TabPane key="director" tab="导演手册" /></Tabs><Table :columns="columns" :data-source="filtered" :loading="loading" row-key="id"><template #bodyCell="{column,record}"><template v-if="column.key === 'items'"><Tag>{{ record.data?.length ?? 0 }}</Tag></template><template v-if="column.key === 'images'">{{ record.images?.length ?? 0 }} 张</template><template v-if="column.key === 'updateTime'">{{ new Date(record.updateTime).toLocaleString() }}</template><template v-if="column.key === 'action'"><Button type="link" @click="edit(record)">编辑</Button><Popconfirm title="确认删除该创作手册？" @confirm="remove(record)"><Button danger type="link">删除</Button></Popconfirm></template></template></Table></Card><Modal v-model:open="open" :title="`${form.id ? '编辑' : '新建'}${form.kind === 'visual' ? '视觉' : '导演'}手册`" width="900px" @ok="save"><Form layout="vertical"><div class="grid grid-cols-2 gap-4"><Form.Item label="显示名称" required><Input v-model:value="form.name" /></Form.Item><Form.Item label="唯一标识" required><Input v-model:value="form.path" :disabled="!!form.id" placeholder="英文或拼音，不能包含路径符号" /></Form.Item></div><Form.Item label="参考图 URL（每行一个）"><Input.TextArea v-model:value="form.imagesText" :rows="3" /></Form.Item><Tabs><Tabs.TabPane v-for="item in form.data" :key="item.value" :tab="item.label"><Input.TextArea v-model:value="item.data" :rows="12" :placeholder="`${item.label} Markdown 内容`" /></Tabs.TabPane></Tabs></Form></Modal></Page></template>
